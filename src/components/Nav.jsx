  ...
  return (
    <>
      <nav className=' h-[10vh] flex justify-between items-center px-[40px] shadow-sm bg-white '>
        ...
        <div className={`sm:hidden  bg-white w-[80%] h-full top-0 right-0 float-right pr-[20px] pt-[20px] z-10  fixed ${isOpen ? 'right-0' : 'right-[-100%]'} transition-all ease-in-out`} >
          <div onClick={toggleOpen} className='flex  text-secondary text-[30px] float-right '>< BiX /></div>
          <div className='flex flex-col text-xl w-full h-full  justify-start items-center mt-[30%] gap-3'>
            <Link to='/'> Home</Link>
            <Link to='/about'> About</Link>
            <Link to='/contact' className='btn flex justify-center items-center gap-3'> Contact <BiPhone /> </Link>
          </div>
